...
function Editor(props) {
  ...
  const prettierFormatWithCursor = (parser, plugins) => {
    try {
      const { formatted, cursorOffset } = prettier.formatWithCursor(
        ...
        {
          cursorOffset: cmRef.doc.indexFromPos(cmRef.current.doc.getCursor()),
          parser,
          plugins
        }
      );
  ...
  useEffect(() => {
    ...
    cmRef.current.on('keydown', (cm, event) => {
      ...
      if (
        ...
        showHint(cmRef.current);
      }
      if (event.key === 'Escape') {
        event.preventDefault();
        const selections = cmRef.current.listSelections();
        if (selections.length > 1) {
          const firstPos = selections[0].head || selections[0].anchor;
          cmRef.current.setSelection(firstPos);
          cmRef.current.scrollIntoView(firstPos);
        } else {
          cmRef.current.getInputField().blur();
        }
      }
    });

    cmRef.current.getWrapperElement().style[
      'font-size'
    ] = `${props.fontSize}px`;
  ...
  useEffect(() => {
    ...
    docsRef.current[props.file.id] = oldDoc;
    cmRef.current.focus();

    if (!props.unsavedChanges) {
      setTimeout(() => props.setUnsavedChanges(false), 400);
    }
  }, [props.file.id, props.unsavedChanges]);
